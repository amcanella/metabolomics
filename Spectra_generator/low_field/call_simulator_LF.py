...
for j in range(1,11):
    ...
    for i in range(SAMPLES):
        ...
        spectrum[i], conc[i], aligned[i] = s.constructor(mets, noise)  #, aligned[i]
    ...
    s.csv_gen( f'low_field/x_{j}_GLYBIEN.csv', NFREQ, spectrum)
    s.csv_gen( f'low_field/y_met_{j}_GLYBIEN.csv', 67, conc) #leave as 67
    
    s.csv_gen( f'low_field/y_aligned_{j}_GLYBIEN.csv', NFREQ, aligned)
    print('----------------------------------------------------')
